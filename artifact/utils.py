...
from sklearn.model_selection import StratifiedKFold, RepeatedStratifiedKFold, StratifiedShuffleSplit, \
    RandomizedSearchCV, cross_val_score, GridSearchCV, cross_validate, train_test_split
from sklearn.metrics import cohen_kappa_score, fbeta_score, make_scorer, confusion_matrix, classification_report, \
    mean_squared_error
...
def extract_LF(Data, nlp1):
    ...
    Data['repeatedPatterns'] = Data.apply(lambda x: repeatedPatterns(x['Candidate Antecedent'].root, x['Pronoun']),
                                          axis=1)
...
def prepare_data_to_savev2(dataframe):
    ...
    dataframe['CandidateAntecedent'] = dataframe.apply(
        lambda x: [x['CandidateAntecedent'].start, x['CandidateAntecedent'].end], axis=1)
...
def loadSavedData(dataframe):
    ...
    dataframe['Candidate Antecedent'] = dataframe.apply(
        lambda x: x['Context'][x['Candidate Antecedent'][0]:x['Candidate Antecedent'][1]], axis=1)
...
def loadSavedDatav2(dataframe):
    ...
    dataframe['CandidateAntecedent'] = dataframe.apply(
        lambda x: x['Context'][x['CandidateAntecedent'][0]:x['CandidateAntecedent'][1]], axis=1)
...
def scoringArgMax(clf, x, y, Class, MultiClass=False):
    ...
    if MultiClass:
        ...
        for i in range(0, len(keys) - 3):
            ...
            scores.append(
                [float("{:.3f}".format(precision)), float("{:.3f}".format(recall)), float("{:.3f}".format(accuracy))])
        ...
        accuracy = cr['accuracy']
        return [float("{:.3f}".format(precision)), float("{:.3f}".format(recall)),
                float("{:.3f}".format(accuracy))], str(Class)
...
def ScoringFB(clf, x, y, Class, MultiClass=False, B=1):
    ...
    if MultiClass:
        ...
        for i in range(0, len(keys) - 3):
            ...
            scores.append(
                [float("{:.3f}".format(precision)), float("{:.3f}".format(recall)), float("{:.3f}".format(FB))])
            classes.append(keys[i])
        return scores
    else:
        precision = cr[str(Class)]['precision']
...
def newevalCV(clf, Xids, Yids, x, y, sampler=None, cv=10, log=False, Train_score=False, Class="Maybe", MultiClass=False,
              B=1, bi=False):
    cv_score = [0., 0., 0.]
    model = clf
...
def groupTTsplit(x, y, group, ratio, random_state=42):
    ...
    for Id in group:
        if 0 in y[x[x.Id == Id].index].unique():
            ...
            Yid.append(1)
    X_train, X_test, y_train, y_test = train_test_split(group, Yid, test_size=ratio, random_state=random_state,
                                                        stratify=Yid)
...
def bert_cv(model, x, y, cv=10, Train_score=False, Class="Ambiguous", verbose=True, epochs=1, batch_size=64):
    ...
    for train_idx, test_idx in skf.split(x[0], y):
        ...
        if Train_score:
            ...
            if verbose:
                print("iteration", i)
                print("Train scores === precision", train_scores[0], "recall", train_scores[1], "accuracy",
                      train_scores[2])
    ...
    if verbose:
        if Train_score:
            print("AVG Train scores with", str(cv) + "fold CV: precision", final_train_scores[0], "recall",
                  final_train_scores[1], "accuracy", final_train_scores[2])
        print("AVG Test scores with", str(cv) + "fold CV: precision", final_test_scores[0], "recall",
              final_test_scores[1], "accuracy", final_test_scores[2])
